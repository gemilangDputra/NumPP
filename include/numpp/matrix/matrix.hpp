...
namespace numpp {
    ...
    class matrix : public matrix_base<matrix<T>, T> {
        ...
        public:
            ...
            static matrix rand(size_t row, size_t col, const T& min=T{0}, const T& max=T{1}, layout order=layout::rowmajor);

            template<RandomEngine RNG>
            static matrix randint(size_t row, size_t col, int64_t min, int64_t max, layout order, RNG& rng);
            static matrix randint(size_t row, size_t col, int64_t min=0, int64_t max=1, layout order=layout::rowmajor);
            ...
            template<matrix_like EXPR>
            static matrix rand_like(const EXPR& other, const T& min=T{0}, const T& max=T{1});
            template<matrix_like EXPR>
            static matrix randint_like(const EXPR& other, int64_t min=0, int64_t max=1);
            template<matrix_like EXPR>
